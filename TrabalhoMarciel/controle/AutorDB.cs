...
namespace TrabalhoMarciel.controle
{
    public class AutorDB
    {
        ...
        public static bool setAlteraAutor(NpgsqlConnection conexao, Autor autor)
        {
            ...
            try
            {
                string sql = "update Autor set autnome = @autnome where autcodigo = @autcodigo;";
                NpgsqlCommand cmd = new NpgsqlCommand();
                cmd.Connection = conexao;
                cmd.CommandText = sql;
        ...
        public static Autor getAutor(NpgsqlConnection conexao, int autcodigo)
        {
            ...
            try
            {
                string sql = "select * from autor where autcodigo = @aut_codigo;";
                NpgsqlCommand cmd = new NpgsqlCommand();
                cmd.Connection = conexao;
                cmd.CommandText = sql;
                cmd.Parameters.Add("@aut_codigo", NpgsqlTypes.NpgsqlDbType.Integer).Value = autcodigo;
